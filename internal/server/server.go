...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/terms", s.list)
	s.mux.HandleFunc("POST /api/terms", s.create)
	s.mux.HandleFunc("GET /api/terms/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/terms/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-glossary", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"terms": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Term{} }
	wj(w, map[string]any{"terms": items, "count": len(items)})
}
