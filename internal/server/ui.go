...
async function load(){var r=await fetch(A+'/terms').then(function(r){return r.json()});items=r.terms||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.definition||'').toLowerCase().includes(q)||(i.category||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No terms.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1">';
h+='<div class="item-title">'+esc(i.name)+'</div>';
if(i.definition)h+='<div class="item-sub">'+esc(i.definition)+'</div>';
...
if(i.category)h+='<span>'+esc(String(i.category))+'</span>';
if(i.aliases)h+='<span>'+esc(String(i.aliases))+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
h+='</div>';});
document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/terms/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:'',definition:'',category:'',aliases:''};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' TERM</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name||'')+'" ></div>';
h+='<div class="fr"><label>Definition</label><input id="f-definition" value="'+esc(i.definition||'')+'" ></div>';
h+='<div class="fr"><label>Category</label><input id="f-category" value="'+esc(i.category||'')+'" ></div>';
h+='<div class="fr"><label>Aliases</label><input id="f-aliases" value="'+esc(i.aliases||'')+'" ></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),definition:document.getElementById('f-definition').value.trim(),category:document.getElementById('f-category').value.trim(),aliases:document.getElementById('f-aliases').value.trim()};
if(editId){await fetch(A+'/terms/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/terms',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
